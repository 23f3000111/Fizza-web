...
const FAQS_EN: { q: string; a: React.ReactNode }[] = [
  { q: "What is a Registered Estate Negotiator (REN)?", a: <>A <strong>Registered Estate Negotiator (REN)</strong> is a licensed property professional registered with BOVAEP. A REN must operate under a registered estate agency firm — in Fizaa&apos;s case, <strong>Esprit Estate Agent Sdn Bhd</strong>. The REN number (<strong>REN 63161</strong>) is verifiable on the BOVAEP public register at <a className="text-brass-2 font-semibold" href="https://www.lppeh.gov.my/" target="_blank" rel="noopener noreferrer">lppeh.gov.my</a>.</> },
  { q: "What types of property does Fizaa specialise in?", a: <>Fizaa focuses on <strong>industrial and commercial real estate</strong> — detached and semi-detached factories, superlink warehouses, industrial land, hotel assets, commercial office floors and built-to-rent (BTR) developments. She does not handle residential property, meaning her knowledge in the industrial-commercial space is deeper than a generalist agent.</> },
  { q: "How do I find the right factory or warehouse for my business?", a: <>Start by identifying your <strong>operational requirements</strong>: floor area, eave height, power supply (amp), loading access, zoning type and location relative to your supply chain. Share these with Fizaa and she&apos;ll match your brief against available listings — including off-market properties. A site visit is always arranged before any commitment.</> },
  ...
  { q: "How do I book a site visit with Fizaa?", a: <>The easiest way is to <strong>WhatsApp Fizaa</strong> at <a className="text-brass-2 font-semibold" href={SITE.whatsapp} target="_blank" rel="noopener noreferrer">{SITE.phone}</a>, or use the <Link className="text-brass-2 font-semibold" href="/quote">Get a Quote</Link> form. Describe the property you need, your budget and timeline. Fizaa will respond promptly, match you to relevant listings and arrange a <strong>private site visit</strong>.</> },
];

const FAQS_BM: { q: string; a: React.ReactNode }[] = [
  { q: "Apakah itu Perunding Hartanah Berdaftar (REN)?", a: <>Seorang <strong>Perunding Hartanah Berdaftar (REN)</strong> ialah profesional hartanah berlesen yang berdaftar dengan BOVAEP. Seorang REN mesti beroperasi di bawah firma agensi hartanah berdaftar — dalam kes Fizaa, <strong>Esprit Estate Agent Sdn Bhd</strong>. Nombor REN (<strong>REN 63161</strong>) boleh disahkan dalam daftar awam BOVAEP di <a className="text-brass-2 font-semibold" href="https://www.lppeh.gov.my/" target="_blank" rel="noopener noreferrer">lppeh.gov.my</a>.</> },
  { q: "Jenis hartanah apakah yang menjadi kepakaran Fizaa?", a: <>Fizaa tertumpu pada <strong>hartanah perindustrian dan komersial</strong> — kilang berkembar dan sesebuah, gudang superlink, tanah perindustrian, aset hotel, tingkat pejabat komersial dan pembangunan built-to-rent (BTR). Beliau tidak mengendalikan hartanah kediaman, bermakna pengetahuannya dalam ruang perindustrian-komersial lebih mendalam berbanding ejen umum.</> },
  { q: "Bagaimana saya mencari kilang atau gudang yang sesuai untuk perniagaan saya?", a: <>Mulakan dengan mengenal pasti <strong>keperluan operasi</strong> anda: luas lantai, ketinggian bumbung, bekalan kuasa (amp), akses memunggah, jenis zon dan lokasi berbanding rantaian bekalan anda. Kongsikan dengan Fizaa dan beliau akan memadankan keperluan anda dengan senarai yang ada — termasuk hartanah luar pasaran. Lawatan tapak sentiasa diatur sebelum sebarang komitmen.</> },
  ...
  { q: "Bagaimana saya menempah lawatan tapak dengan Fizaa?", a: <>Cara paling mudah ialah <strong>WhatsApp Fizaa</strong> di <a className="text-brass-2 font-semibold" href={SITE.whatsapp} target="_blank" rel="noopener noreferrer">{SITE.phone}</a>, atau gunakan borang <Link className="text-brass-2 font-semibold" href="/quote">Dapatkan Sebut Harga</Link>. Terangkan hartanah yang anda perlukan, bajet dan jangka masa anda. Fizaa akan membalas dengan segera, memadankan anda dengan senarai yang berkaitan dan mengatur <strong>lawatan tapak peribadi</strong>.</> },
];

const FAQS_ZH: { q: string; a: React.ReactNode }[] = [
  { q: "什么是注册地产协商员（REN）？", a: <><strong>注册地产协商员（REN）</strong>是在 BOVAEP 注册的持牌地产专业人士。REN 必须隶属于一家注册地产代理行执业——以 Fizaa 而言，即 <strong>Esprit Estate Agent Sdn Bhd</strong>。其 REN 编号（<strong>REN 63161</strong>）可在 BOVAEP 公开名册查证：<a className="text-brass-2 font-semibold" href="https://www.lppeh.gov.my/" target="_blank" rel="noopener noreferrer">lppeh.gov.my</a>。</> },
  { q: "Fizaa 专精于哪些类型的物业？", a: <>Fizaa 专注于<strong>工业与商业地产</strong>——独立式与半独立式厂房、超级连排仓库、工业地段、酒店资产、商业办公楼层以及订制出租（BTR）项目。她不经手住宅物业，因此在工业与商业领域的专业深度远超全能型经纪。</> },
  { q: "我该如何为公司找到合适的厂房或仓库？", a: <>首先厘清您的<strong>营运需求</strong>：楼面面积、檐高、电力供应（安培）、装卸通道、分区用途，以及相对于供应链的地点。将这些告诉 Fizaa，她会依您的条件匹配现有房源——包括未公开的私洽物业。任何承诺之前，必定先安排实地考察。</> },
  ...
  { q: "关于马来西亚的工业分区，我需要知道什么？", a: <>工业地段分为<strong>轻工业、中工业与重工业</strong>三类。您的营业活动必须符合该地段获批的分区用途——在轻工业地段经营重工业将面临执法风险。在推荐任何工业物业之前，Fizaa 都会核实分区用途、地契限制与 MITI 要求。</> },
  { q: "商业与工业地产的代理费如何计算？", a: <>根据《<em>1981 年估价师、估值师、地产代理与物业经理法令（第 242 号法令）</em>》，<strong>转售的最高佣金为售价的 3%</strong>。租赁方面，佣金通常为 <strong>1.25 个月</strong>租金。两者均需另加现行 SST。在接受任何委托之前，Fizaa 都会先厘清费用结构。</> },
  ...
  { q: "我该如何预约与 Fizaa 一同实地考察？", a: <>最简单的方式是<strong>透过 WhatsApp 联系 Fizaa</strong>：<a className="text-brass-2 font-semibold" href={SITE.whatsapp} target="_blank" rel="noopener noreferrer">{SITE.phone}</a>，或使用<Link className="text-brass-2 font-semibold" href="/quote">获取报价</Link>表格。说明您需要的物业、预算与时间安排，Fizaa 会尽快回复，为您匹配相关房源并安排<strong>私人实地考察</strong>。</> },
];
    ...
    <>
      <section className="navy-gradient text-white pt-8 pb-24">
        <div className="container-site">
          ...
          <p className="text-[#B9C7D8] mt-2.5 max-w-[56ch]">{t("Common questions about industrial & commercial property, REN registration, agent fees and what Fizaa actually does — before you pick up the phone.")}</p>
        </div>
      </section>
      ...
      <div className="container-site grid lg:grid-cols-[1fr_320px] gap-7 -mt-16 mb-[70px] items-start">
        ...
        <aside className="lg:sticky lg:top-[88px]">
          <div className="bg-navy text-white rounded-xl2 p-[26px]">
            ...
            <p className="text-[#B9C7D8] text-sm mb-4">{t("Ask Fizaa directly — she replies personally, usually within minutes.")}</p>
